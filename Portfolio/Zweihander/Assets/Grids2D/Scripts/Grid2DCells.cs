...
namespace Grids2D {
	...
	public partial class Grid2D : MonoBehaviour {
		...
		/// <summary>
		/// Sets additional cost of crossing any hexagon side.
		/// </summary>
		/// <param name="cellIndex">Cell index.</param>
		/// <param name="additionalCost">Additional cost.</param>
		public void CellSetAllSidesCrossCost(int cellIndex, int additionalCost) {
			if (cellIndex < 0 || cellIndex >= cells.Count)
				return;
			cells[cellIndex].SetAllSidesCost(additionalCost);
		}
		...
		/// </summary>
		public GameObject CellToggle(int cellIndex, bool visible, Texture2D texture) {
			return CellToggle(cellIndex, visible, Color.white, false, texture, Misc.Vector2one, Misc.Vector2zero, 0);
		}
		...
		/// </summary>
		public GameObject CellToggle(int cellIndex, bool visible, Color color, bool refreshGeometry, int textureIndex) {
			Texture2D texture = null;
			if (textureIndex >= 0 && textureIndex < textures.Length)
				texture = textures[textureIndex];
			return CellToggle(cellIndex, visible, color, refreshGeometry, texture, Misc.Vector2one, Misc.Vector2zero, 0);
		}

		/// <summary>
		/// Colorize specified region of a cell by indexes.
		/// </summary>
		public GameObject CellToggle(int cellIndex, bool visible, Color color, bool refreshGeometry, Texture2D texture) {
			return CellToggle(cellIndex, visible, color, refreshGeometry, texture, Misc.Vector2one, Misc.Vector2zero, 0);
		}
		...
		public GameObject CellToggle(int cellIndex, bool visible, Color color, bool refreshGeometry, Texture2D texture, Vector2 textureScale, Vector2 textureOffset, float textureRotation) {
			return CellToggle(cellIndex, visible, color, refreshGeometry, texture, textureScale, textureOffset, textureRotation, false);
		}

		/// <summary>
		/// Colorize specified region of a territory by indexes.
		/// </summary>
		public GameObject CellToggle(int cellIndex, bool visible, Color color, bool refreshGeometry, Texture2D texture, Vector2 textureScale, Vector2 textureOffset, float textureRotation, bool rotateInLocalSpace) {
			if (cellIndex < 0 || cellIndex >= cells.Count)
				return null;
		...
		/// </summary>
		public List<int> CellGetNeighbours(int cellIndex, int distance, int maxCost = -1, int cellGroupMask = -1) {
			if (cellIndex < 0 || cellIndex >= cells.Count)
				return null;
			Cell cell = cells[cellIndex];
